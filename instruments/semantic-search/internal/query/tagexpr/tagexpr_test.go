...
func TestParseAndEval(t *testing.T) {
	cases := []struct {
		...
		{
			name: "case insensitive whole segment",
			expr: "Project",
			tags: []string{"project/alpha"},
			want: true,
		},
		{
			name: "atom matches path prefix segment",
			expr: "foo",
			tags: []string{"foo/bar/baz"},
			want: true,
		},
		{
			name: "atom matches inner segment",
			expr: "bar",
			tags: []string{"foo/bar/baz"},
			want: true,
		},
		{
			name: "partial word does not match (proj vs project)",
			expr: "proj",
			tags: []string{"project/alpha"},
			want: false,
		},
		{
			name: "cyrillic exact tag matches",
			expr: "Ли",
			tags: []string{"Ли"},
			want: true,
		},
		{
			name: "cyrillic substring within word does not match",
			expr: "Ли",
			tags: []string{"мысли"},
			want: false,
		},
		{
			name: "multi-segment atom matches contiguous segments",
			expr: "foo/bar",
			tags: []string{"a/foo/bar/b"},
			want: true,
		},
		{
			name: "multi-segment atom non-contiguous does not match",
			expr: "foo/baz",
			tags: []string{"foo/bar/baz"},
			want: false,
		},
